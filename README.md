...
**Canlı ortam:** K3s kümesi · Docker Hub `since1907/akgun-backend:v1.8.26` · `since1907/akgun-frontend:v1.8.61`  
...
\* **Renk sütunu** (v1.8.51): eski şema rengi `Aciklama` sonuna `| Renk: Black` olarak yazıyordu. Dışa aktarım bu artığı okuyup `Renk` sütununa taşır ve açıklamadan çıkarır; migration aynı taşımayı veritabanında kalıcı yapar.
...
| v1.8.61 | F2 listesinde marka·model ayrı satır; fiş/sepet ürün adı artık kırpılmaz (stok kartındaki ad olduğu gibi) |
| v1.8.59 / API v1.8.26 | Üst barda döviz çevirici; il/ilçe seçimi; Ana Sayfa hızlı işlemler (Ödeme Al dahil); fişte sade stok adı (marka/model ön eki gizlenir); stok kartında marka/model ayrı kaydedilir; Excel Satis1 / Satis2 sütunları (SatisFiyati dışa verilmez; etiketler v1.14.1'de düzeltildi: Satış 1 = toptan, Satış 2 = perakende); iade ve ekstre iyileştirmeleri |
...
| v1.8.49 / API v1.8.21 | F2 aramada **relevans sıralaması** — "Note 8" araması artık gerçek Note 8 ürünlerini başa alır (önceki sıralama salt alfabetikti, "NOTE 10..." öne geçiyordu); çok kelimeli aramada 1-2 karakterlik parçalar yalnızca ad/marka/model içinde aranır (stok kodundaki rakama denk gelmez); Türkçe katlama düzeltmesi (INFİNİX ↔ infinix) |
...
| v1.8.43 | İadede de genel müşteri (kod 120) seçilince fatura tipi Kapalı; diğerlerinde Açık |
| v1.8.42 | Satışta genel müşteri (kod 120) seçilince ödeme varsayılanı Kapalı/Nakit; diğer müşterilerde Açık/Cari |
| v1.8.41 / API v1.8.17 | F2 hızlı arama (yerel daraltma + oturum koruma); fişte müşteri detayı ve mali değersiz uyarı; satışta sonradan müşteri seçince fiyat değişmez; adetler tam sayı; fiş no YYMMDDHHmmss; kayıt sonrası sepet/müşteri kalır; ekstre yeni sekme; işlem yapan varsayılan Seçiniz; iade Açık/Kapalı fatura (cari veya kasadan çıkış) |
| v1.8.40 / API v1.8.16 | Model arama düzeltmesi (stok + tanım birleşik); sidebar sürüm etiketi; önbellek iyileştirmesi |

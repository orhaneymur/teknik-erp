/**
 * Türkiye idari bölünüşü — 81 il ve bağlı ilçeler.
 * Dış servis çağrısı yok; liste uygulama paketine gömülüdür (çevrimdışı çalışır).
 * Büyükşehirlerde "Merkez" yoktur, merkez ilçeler kendi adlarıyla yer alır.
 */

/** İl adı → virgülle ayrılmış ilçe listesi (paket boyutu için düz metin) */
const RAW_DISTRICTS: Record<string, string> = {
  Adana:
...
/** Türkçe alfabetik sıralama (ç, ğ, ı, ö, ş, ü doğru yerlerine gelir) */
const trCollator = new Intl.Collator('tr-TR', { sensitivity: 'base' });

export const TURKEY_PROVINCES: string[] = Object.keys(RAW_DISTRICTS).sort((a, b) =>
  trCollator.compare(a, b)
);

/** İl adı → sıralı ilçe dizisi */
export const TURKEY_DISTRICTS: Record<string, string[]> = Object.fromEntries(
  Object.entries(RAW_DISTRICTS).map(([province, districts]) => [
    province,
    districts.split(',').sort((a, b) => trCollator.compare(a, b)),
  ])
);

/** Girilen il adını resmî yazımına eşler; bulunamazsa null (eski serbest metin kayıtları için) */
export function normalizeProvince(value: string | null | undefined): string | null {
  const trimmed = value?.trim();
  if (!trimmed) return null;
  return (
    TURKEY_PROVINCES.find((province) => trCollator.compare(province, trimmed) === 0) ??
    null
  );
}

/** Bir ilin ilçe listesini döndürür; il tanınmazsa boş dizi */
export function districtsOf(province: string | null | undefined): string[] {
  const normalized = normalizeProvince(province);
  return normalized ? TURKEY_DISTRICTS[normalized] : [];
}

/** Girilen ilçe adını, ili içinde resmî yazımına eşler; bulunamazsa null */
export function normalizeDistrict(
  ...
  if (!trimmed) return null;
  return (
    districtsOf(province).find(
      (district) => trCollator.compare(district, trimmed) === 0
    ) ?? null
  );
}

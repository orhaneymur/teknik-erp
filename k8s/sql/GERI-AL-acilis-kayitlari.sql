...
SELECT 'ONCE' AS asama, COUNT(*) AS kasasiz_acilis FROM Transaction WHERE safeId IS NULL AND (UPPER(description) LIKE '%ESK_ S_STEM%' OR (description = 'AKTARILDI' AND createdAt < '2026-09-12 03:00:00'));
...
UPDATE Safe SET balance = balance + (
  ...
  FROM Transaction WHERE safeId IS NULL AND (UPPER(description) LIKE '%ESK_ S_STEM%' OR (description = 'AKTARILDI' AND createdAt < '2026-09-12 03:00:00'))
) WHERE id = @kasa;

UPDATE Transaction SET safeId = @kasa
WHERE safeId IS NULL AND (UPPER(description) LIKE '%ESK_ S_STEM%' OR (description = 'AKTARILDI' AND createdAt < '2026-09-12 03:00:00'));
